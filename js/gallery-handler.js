...
function load_gallery(_callback) {
  $.getJSON(galleryFolder + "gallery.json", function (data) {
    ...
    for (let i = 0; i < gallerySize; i++) galleryUsed.push(false);
...
function gallery_overlay_change(img_id) {
  ...
  var text = galleryTitles[gallery.indexOf(img_id)] + "<br/>";
...
function fetch_next() {
  for (let i = 0; i < gallerySize; i++) {
    if (!galleryUsed[i]) {
      galleryUsed[i] = true;
      return { "url": gallery[i], "title": galleryTitles[i] };
    }
  }
...
function fetch_random() {
  ...
  var ctr = 0;
  for (let i = 0; i < gallerySize; i++) {
    if (galleryUsed[i]) ctr++;
  }
  if (ctr >= gallerySize) return { "url": "media/oops.png", "title": "An error occured loading this image" };

  // Tries to get a random element of the gallery, trying again if its choice is already being unused
  // TODO Figure out a better system for this, probably by popping already-used elements
  while (1) {
    var r = Math.floor(Math.random() * gallerySize);
    if (!galleryUsed[r]) {
      galleryUsed[r] = true;
      return { "url": gallery[r], "title": galleryTitles[r] };
    }
  }
}
...
function fetch_bg_random() {
  if (bgList.length <= 0) return "media/oops.png";

  var r = Math.floor(Math.random() * bgList.length);
  return bgList[r];
}

function clear_used_list() {
  for (let i = 0; i < gallerySize; i++) galleryUsed[i] = false;
}
